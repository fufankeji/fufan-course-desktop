# FuFan Teaching Agent npm wrapper

> The classroom-focused terminal coding agent adapted from CodeWhale.

FuFan Teaching Agent is a Rust TUI and CLI for classroom programming workflows.
It keeps CodeWhale's provider routing, approval-gated tools, OS sandboxing,
side-git snapshots, and `/restore` rollback, then adds teaching-mode visibility
for course sessions.

This npm package is a small launcher: it downloads the matching native
FuFan Teaching Agent binaries for your platform, verifies them against the release
SHA-256 manifest, and installs the `codewhale`, `codew`, and `codewhale-tui`
commands for compatibility. The application state and credentials still live in
the upstream-compatible config files, not inside `node_modules`.
