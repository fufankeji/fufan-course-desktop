# FuFan Teaching Agent

> A teaching-focused terminal agent adapted from CodeWhale for programming courses.

FuFan Teaching Agent is a Rust TUI and CLI for classroom programming workflows.
It keeps CodeWhale's provider routing, approval-gated tools, OS sandboxing, and
rollback model, then adds teaching-mode instrumentation for visible skill,
context, command, tool-call, safety, and session-export events.

This repository is a FuFan-maintained derivative of the upstream MIT-licensed
[CodeWhale](https://github.com/Hmbown/CodeWhale) project. Upstream attribution
and license details are kept in [LICENSE](LICENSE) and
...
![FuFan Teaching Agent running in a terminal](assets/screenshot.png)

## Install

```bash
npm install -g codewhale
ff-agent --version   # 0.8.61
```

During the current adaptation phase, the published package and binary artifacts
still use the upstream-compatible `codewhale`, `codew`, and `codewhale-tui`
names. The user-facing CLI help and TUI title present the classroom product as
`ff-agent` / `FuFan Teaching Agent TUI`. Prefer building from source? Use cargo
(Rust 1.88+):
...
then run `ff-agent doctor` to confirm.

## First Run

```bash
ff-agent auth set --provider deepseek
ff-agent auth status
ff-agent doctor
ff-agent
```
...
`ff-agent auth set --provider anthropic` — or just export
...
```bash
ff-agent exec --allowed-tools read_file,exec_shell --max-turns 10 "fix the failing test"
```

## What Ships

A terminal-native teaching agent harness — TUI + CLI, 16 Rust crates — where
the safety rails are runtime mechanisms, not advice the model has to remember:

- **Teaching mode events.** Course configuration, student-visible sidebars, and
  JSONL/Markdown exports make skill loading, context assembly, tool calls,
  command classification, safety checks, and session closeout inspectable.
