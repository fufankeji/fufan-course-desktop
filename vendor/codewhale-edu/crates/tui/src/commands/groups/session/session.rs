...
pub fn export(app: &mut App, path: Option<&str>) -> CommandResult {
    if app.education_enabled && path.is_none() {
        return CommandResult::action(AppAction::ExportTeachingSession);
    }
...
mod tests {
    ...
    #[test]
    fn test_export_without_path_uses_teaching_export_in_teaching_mode() {
        let tmpdir = TempDir::new().unwrap();
        let mut app = create_test_app_with_tmpdir(&tmpdir);
        app.education_enabled = true;
        ...
        assert!(matches!(
            result.action,
            Some(AppAction::ExportTeachingSession)
        ));
    }
