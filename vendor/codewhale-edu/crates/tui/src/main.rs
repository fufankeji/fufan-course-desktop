//! CLI entry point for FuFan Teaching Agent.
...
#[command(
    ...
    about = "FuFan Teaching Agent terminal coding assistant",
    long_about = "Teaching-focused terminal TUI and CLI for classroom programming workflows.\n\nRun 'ff-agent' to start.\n\nProvider routes include DeepSeek, Arcee, Hugging Face, OpenRouter, Xiaomi MiMo, local vLLM/SGLang/Ollama, and more."
)]
struct Cli {
    /// Subcommand to run
    #[command(subcommand)]
...
enum Commands {
    ...
    Setup(SetupArgs),
    /// Generate a remote FuFan Teaching Agent deploy bundle (cloud + chat bridge)
    ...
    Exec(ExecArgs),
    /// Generate SWE-bench prediction rows from FuFan Teaching Agent runs
...
#[command(after_help = "\
Examples:
  ff-agent exec \"explain this function\"
  ff-agent exec --auto \"list crates/ with ls\"
  ff-agent exec --auto --output-format stream-json \"fix the failing test\"

Plain `ff-agent exec` is a one-shot model response. Use `--auto` for
...
enum SwebenchCommand {
    /// Run FuFan Teaching Agent on one SWE-bench instance and export the resulting diff
    Run(SwebenchRunArgs),
...
enum McpCommand {
    ...
    Validate,
    /// Register this FuFan Teaching Agent binary as a local MCP stdio server.
    ///
    ...
    #[command(
        name = "add-self",
        long_about = "Register this FuFan Teaching Agent binary as a local MCP stdio server.\n\nAdds a config entry to ~/.codewhale/mcp.json that launches `codewhale serve --mcp`\nvia the stdio transport. Other FuFan Teaching Agent sessions (or any MCP client) can then\ndiscover and call tools exposed by this server.\n\nUse `codewhale serve --http` instead if you need the HTTP/SSE runtime API."
    )]
    AddSelf {
...
fn swebench_prompt(
    ...
    prompt.push_str("Edit the workspace to resolve the issue. Run targeted tests when practical. Do not commit, tag, publish, or change remotes. Leave the final solution as a working-tree diff; FuFan Teaching Agent will export that diff as the SWE-bench prediction.\n\n");
...
async fn run_doctor(config: &Config, workspace: &Path, config_path_override: Option<&Path>) {
    ...
    println!("  ff-agent: {}", env!("DEEPSEEK_BUILD_VERSION"));
...
mod terminal_mode_tests {
    ...
    fn companion_binary_reports_its_own_name() {
        assert_eq!(Cli::command().get_name(), "ff-agent");
    }
