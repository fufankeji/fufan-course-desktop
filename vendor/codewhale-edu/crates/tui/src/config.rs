...
/// Save a complete provider connection to config-file storage.
///
/// This is used by the Runtime API's explicit `persist: true` flow. It updates
/// the active provider and provider-scoped model/base URL/API key without
/// logging or returning secret material.
pub fn save_provider_connection_for(
    provider: ApiProvider,
    model: Option<&str>,
    base_url: Option<&str>,
    api_key: Option<&str>,
    ...
    let providers = table
        ...
        .as_table_mut()
        .context("`providers` must be a table.")?;
    let key_inside = provider_table_key(provider);
    let entry = providers
        .entry(key_inside.to_string())
        .or_insert_with(|| toml::Value::Table(toml::value::Table::new()))
        .as_table_mut()
...
fn provider_table_key(provider: ApiProvider) -> &'static str {
    match provider {
...
"#,
    ...
    #[test]
    fn save_provider_connection_writes_provider_model_base_url_and_key() -> Result<()> {
        let _lock = lock_test_env();
        let nanos = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap()
            .as_nanos();
        let temp_root = env::temp_dir().join(format!(
            "codewhale-tui-save-provider-connection-{}-{}",
            std::process::id(),
            nanos
        ));
        fs::create_dir_all(&temp_root)?;
        ...
        save_provider_connection_for(
            ApiProvider::Novita,
            Some("novita-test-model"),
            Some("https://api.novita.ai/v3/openai"),
            Some("novita-test-key"),
        )?;
    ...
    #[test]
    fn save_provider_connection_supports_deepseek_provider_table() -> Result<()> {
        let _lock = lock_test_env();
        let nanos = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap()
            .as_nanos();
        let temp_root = env::temp_dir().join(format!(
            "codewhale-tui-save-provider-connection-deepseek-{}-{}",
            std::process::id(),
            nanos
        ));
        fs::create_dir_all(&temp_root)?;
        ...
        let path = save_provider_connection_for(
            ApiProvider::Deepseek,
            Some("deepseek-chat"),
