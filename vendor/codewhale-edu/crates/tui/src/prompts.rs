...
你正在赋范智能体中运行。无论任务上下文（代码、错误日志、文件名）\
...
FuFan Teaching Agent を実行しています。タスクコンテキスト（コード、エラーログ、\
...
Você está rodando dentro do FuFan Teaching Agent. Escreva tanto \
...
Bạn đang chạy trong FuFan Teaching Agent. Cho dù ngữ cảnh tác vụ (mã nguồn, nhật ký lỗi, tên tệp) \
...
The FuFan Teaching Agent Constitution governs your behavior. Ground truth is the
...
mod tests {
    ...
    #[test]
    fn default_model_visible_identity_is_fufan_teaching_agent() {
        let static_prompt =
            compose_prompt_with_approval_model_and_shell(Personality::Calm, "deepseek-v4-flash");
        assert!(
            static_prompt.contains("## CONSTITUTION OF FUFAN TEACHING AGENT"),
            "base prompt must carry FuFan Teaching Agent identity"
        );
        assert!(
            !static_prompt.contains("## CONSTITUTION OF CODEWHALE"),
            "base prompt must not carry CodeWhale identity"
        );

        let tmp = tempdir().expect("tempdir");
        let full_prompt = match system_prompt_for_mode_with_context_skills_session_and_approval(
            tmp.path(),
            None,
            None,
            None,
        ...
        assert!(
            full_prompt.contains("The FuFan Teaching Agent Constitution governs your behavior"),
            "authority recap must use FuFan Teaching Agent identity"
        );
        assert!(
            full_prompt.contains("你正在赋范智能体中运行"),
            "Chinese locale bookend must use 赋范智能体 identity"
        );
        for stale_identity in [
            "The Constitution of CodeWhale governs your behavior",
            "## CONSTITUTION OF CODEWHALE",
            "你正在 codewhale 中运行",
        ] {
            assert!(
                !full_prompt.contains(stale_identity),
                "full prompt leaked stale identity: {stale_identity}"
            );
        }
    }
    ...
    fn authority_recap_appears_in_full_prompt() {
        ...
        assert!(
            text.contains("The FuFan Teaching Agent Constitution governs your behavior"),
    ...
    fn environment_block_is_inserted_into_system_prompt() {
        ...
        assert!(prompt.contains("- agent_version:"));
    }
    ...
    fn constitution_md_carries_required_structure() {
        let md = BASE_PROMPT;
        assert!(
            md.contains("## CONSTITUTION OF FUFAN TEACHING AGENT"),
            "missing title"
        );
