...
const TEACHING_SIDEBAR_VISIBLE_MIN_WIDTH: u16 = 120;
...
fn teaching_sidebar_visible(
    education_enabled: bool,
    state: &crate::education::state::EducationState,
    chat_width: u16,
) -> bool {
    education_enabled
...
fn split_teaching_sidebar_for_chat_area(
    ...
    education_enabled: bool,
    state: &crate::education::state::EducationState,
    existing_sidebar_visible: bool,
) -> (Rect, Option<Rect>) {
    if existing_sidebar_visible
        || !teaching_sidebar_visible(education_enabled, state, chat_area.width)
    {
...
pub async fn run_tui(config: &Config, options: TuiOptions) -> Result<()> {
    ...
    if let Err(err) = app.finalize_teaching_session() {
        tracing::warn!(target: "education.export", ?err, "failed to finalize teaching session");
    }
    ...
    loop {
        ...
        let mut fallback_after_engine_error: Option<ApiProvider> = None;
        {
            ...
            for _ in 0..MAX_ENGINE_EVENTS_PER_DRAIN {
                ...
                let education_session_id = app.current_session_id.clone();
                app.education_observer
                    .observe_engine_event(&event, education_session_id.as_deref());
...
async fn apply_command_result(
    ...
    if let Some(action) = result.action {
        match action {
            ...
            AppAction::ExportTeachingSession => match app.export_teaching_session() {
                Ok(path) => {
                    app.status_message =
                        Some(format!("Teaching session exported to {}", path.display()));
                }
                Err(err) => {
                    app.status_message = Some(format!("Teaching export failed: {err}"));
                }
            },
...
fn render(f: &mut Frame, app: &mut App) {
    ...
    // Render chat + sidebar + optional file-tree pane
    {
        ...
        let (split_chat_area, split_teaching_sidebar_area) = split_teaching_sidebar_for_chat_area(
            chat_area,
            app.education_enabled,
            &app.education_state,
            sidebar_area.is_some(),
        );
        chat_area = split_chat_area;
        let teaching_sidebar_area = split_teaching_sidebar_area;
        ...
        if let Some(teaching_sidebar_area) = teaching_sidebar_area {
            crate::education::sidebar::render_teaching_sidebar(
                f,
                teaching_sidebar_area,
                &app.education_state,
            );
        }
    }
