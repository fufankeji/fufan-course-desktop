...
fn build_escape(method: Method, in_tmux: bool, msg: &str) -> Vec<u8> {
    match method {
        ...
        Method::Ghostty => {
            ...
            let seq = format!("\x1b]777;notify;ff-agent;{msg}\x07");
...
pub fn stop_title_animation() {
    ...
    if mode == 1 {
        set_terminal_title("✅ FuFan Teaching Agent");
    }
    ...
    let _ = std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            let args = [
                ...
                "-e".to_string(),
                "display notification theBody with title \"FuFan Teaching Agent\" subtitle theSubtitle sound name \"default\"".to_string(),
                "-e".to_string(),
                "end run".to_string(),
                "--".to_string(),
                body,
...
fn macos_notification_parts(msg: &str) -> (String, String) {
    ...
    if lines.is_empty() {
        return ("FuFan Teaching Agent".to_string(), String::new());
    }
...
mod tests {
    ...
    #[test]
    fn osc9_body_format() {
        let out = capture(Method::Osc9, false, "ff-agent: done", 0, 1);
        assert_eq!(out, b"\x1b]9;ff-agent: done\x07");
    }
    ...
    fn ghostty_escape_format() {
        ...
        let s = String::from_utf8(out).unwrap();
        assert!(
            s.contains("777;notify;ff-agent;done"),
