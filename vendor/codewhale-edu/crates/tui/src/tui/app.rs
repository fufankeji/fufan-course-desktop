...
use crate::education::observer::EducationObserver;
...
fn teaching_mode_enabled_for_workspace(workspace: &Path) -> bool {
    workspace.join("course.yaml").is_file()
        || workspace.join("course.yml").is_file()
        || workspace.join(".codewhale").join("course.yaml").is_file()
        || workspace.join(".codewhale").join("course.yml").is_file()
}
...
pub struct App {
    ...
    /// Education observer for teaching sidebar and session exports.
    pub education_observer: EducationObserver,
    /// Whether teaching-mode UI surfaces should be rendered.
    pub education_enabled: bool,
    /// Recent education events projected for the teaching sidebar.
...
impl App {
    ...
    pub fn new(options: TuiOptions, config: &Config) -> Self {
        ...
        let mcp_configured_count =
            crate::mcp::load_config_with_workspace(&mcp_config_path, &workspace)
                ...
                .unwrap_or(0);
        let education_export_root = workspace.join("exports").join("sessions");
        let education_enabled = teaching_mode_enabled_for_workspace(&workspace);
        Self {
            mode: initial_mode,
            ...
            education_observer: EducationObserver::with_export_root(education_export_root),
            education_enabled,
    ...
    pub fn export_teaching_session(&mut self) -> std::io::Result<PathBuf> {
        let current_session_id = self.current_session_id.clone();
        let session_id = self
            .education_observer
            .session_id_for_export(current_session_id.as_deref());
        let output_dir = self
            .workspace
            .join("exports")
            .join("sessions")
            .join(&session_id);
        let report_path = output_dir.join("session-report.md");
        let display_report_path = self.display_export_path(&report_path);

        self.education_observer.emit_manual_event(
            Some(&session_id),
            crate::education::events::EducationEventType::ExportCreated,
            ...
            format!("Export created at {display_report_path}"),
            serde_json::json!({
                "path": display_report_path,
            }),
        );
        self.education_state = self.education_observer.state().clone();

        self.education_observer.flush()?;
        let jsonl_path = self.education_observer.jsonl_path().ok_or_else(|| {
            std::io::Error::new(
                std::io::ErrorKind::NotFound,
                "session.jsonl writer unavailable",
            )
        })?;
        let events = crate::education::report::read_jsonl_events(jsonl_path)?;
        let path = crate::education::report::write_markdown_report(&output_dir, None, &events)?;
        Ok(path)
    }

    fn display_export_path(&self, path: &Path) -> String {
        if let Ok(relative) = path.strip_prefix(&self.workspace) {
            return format!("./{}", relative.display());
        }
        crate::education::redaction::redact_text(&path.display().to_string())
    }

    pub fn finalize_teaching_session(&mut self) -> std::io::Result<Option<PathBuf>> {
        ...
        self.education_observer.emit_manual_event(
            Some(&session_id),
            crate::education::events::EducationEventType::SessionEnded,
            crate::education::events::EducationActor::System,
            crate::education::events::EducationVisibility::Student,
            crate::education::events::EducationSeverity::Info,
...
pub enum AppAction {
    ...
    SendMessage(String),
    /// Export the current teaching session JSONL and Markdown report.
    ExportTeachingSession,
    ListSubAgents,
    FetchModels,
    CacheWarmup,
...
mod tests {
    ...
    #[test]
    fn teaching_mode_enables_when_workspace_has_course_yaml() {
        let dir = tempfile::tempdir().expect("tempdir");
        assert!(!teaching_mode_enabled_for_workspace(dir.path()));

        std::fs::write(dir.path().join("course.yaml"), "id: demo\n").expect("write course yaml");
        assert!(teaching_mode_enabled_for_workspace(dir.path()));

        std::fs::remove_file(dir.path().join("course.yaml")).expect("remove course yaml");
        std::fs::create_dir_all(dir.path().join(".codewhale")).expect("create config dir");
        std::fs::write(
            dir.path().join(".codewhale").join("course.yml"),
            "id: demo\n",
        )
        .expect("write nested course yaml");
        assert!(teaching_mode_enabled_for_workspace(dir.path()));
    }

    #[test]
    fn export_teaching_session_writes_jsonl_and_markdown_report() {
        let dir = tempfile::tempdir().expect("tempdir");
        let mut options = test_options(false);
        options.workspace = dir.path().to_path_buf();
        std::fs::write(dir.path().join("course.yaml"), "id: demo\n").expect("write course yaml");
        let mut app = App::new(options, &Config::default());
        app.current_session_id = Some("sess_export".to_string());

        app.education_observer.emit_manual_event(
            app.current_session_id.as_deref(),
            crate::education::events::EducationEventType::ShellFinished,
            crate::education::events::EducationActor::Agent,
            crate::education::events::EducationVisibility::Student,
            crate::education::events::EducationSeverity::Info,
            "cargo test exited with code 0",
            serde_json::json!({"command": "cargo test", "exit_code": 0}),
        );
        app.education_state = app.education_observer.state().clone();

        let report_path = app
            .export_teaching_session()
            .expect("export teaching session");
        ...
        assert_eq!(
            ...
            dir.path()
                ...
                .join("sessions")
                .join("sess_export")
                .join("session-report.md")
        );
        let jsonl_path = report_path
            .parent()
            .expect("report parent")
            .join("session.jsonl");
        let jsonl = std::fs::read_to_string(jsonl_path).expect("read jsonl");
        assert!(jsonl.contains("\"type\":\"shell-finished\""));
        assert!(jsonl.contains("\"type\":\"export-created\""));
        let report = std::fs::read_to_string(report_path).expect("read report");
        assert!(report.contains("cargo test exited with code 0"));
        assert!(report.contains("export.created"));
    }

    #[test]
    fn export_teaching_session_report_uses_full_jsonl_not_sidebar_cache() {
        let dir = tempfile::tempdir().expect("tempdir");
        let mut options = test_options(false);
        options.workspace = dir.path().to_path_buf();
        std::fs::write(dir.path().join("course.yaml"), "id: demo\n").expect("write course yaml");
        let mut app = App::new(options, &Config::default());
        app.current_session_id = Some("sess_long".to_string());

        for idx in 0..205 {
            app.education_observer.emit_manual_event(
                app.current_session_id.as_deref(),
                crate::education::events::EducationEventType::ToolStarted,
                crate::education::events::EducationActor::Agent,
                crate::education::events::EducationVisibility::Student,
                crate::education::events::EducationSeverity::Info,
                format!("event {idx:03}"),
        ...
        app.education_state = app.education_observer.state().clone();
        assert_eq!(app.education_state.len(), 200);
        ...
        let report = std::fs::read_to_string(report_path).expect("read report");
        assert!(report.contains("event 000"));
        assert!(report.contains("- 事件总数：206"));
    }

    #[test]
    fn export_teaching_session_redacts_export_path_in_jsonl_data() {
        let dir = tempfile::tempdir().expect("tempdir");
        let mut options = test_options(false);
        options.workspace = dir.path().to_path_buf();
        std::fs::write(dir.path().join("course.yaml"), "id: demo\n").expect("write course yaml");
        let mut app = App::new(options, &Config::default());
        app.current_session_id = Some("sess_redact".to_string());
        ...
        assert!(!jsonl.contains(&dir.path().display().to_string()));
        assert!(jsonl.contains("./exports/sessions/sess_redact/session-report.md"));
    }

    #[test]
    fn export_teaching_session_fails_when_jsonl_writer_is_unavailable() {
        let dir = tempfile::tempdir().expect("tempdir");
        let mut options = test_options(false);
        options.workspace = dir.path().to_path_buf();
        std::fs::write(dir.path().join("course.yaml"), "id: demo\n").expect("write course yaml");
        let mut app = App::new(options, &Config::default());
        app.current_session_id = Some("sess_blocked".to_string());
        std::fs::create_dir_all(dir.path().join("exports").join("sessions"))
            .expect("create exports dir");
        std::fs::write(
            dir.path()
                .join("exports")
                .join("sessions")
        ...
        let error = app
            .export_teaching_session()
            .expect_err("writer failure should fail export");
    ...
    #[test]
    fn finalize_teaching_session_emits_session_end_and_report() {
        let dir = tempfile::tempdir().expect("tempdir");
        let mut options = test_options(false);
        options.workspace = dir.path().to_path_buf();
        std::fs::write(dir.path().join("course.yaml"), "id: demo\n").expect("write course yaml");
        let mut app = App::new(options, &Config::default());
        app.current_session_id = Some("sess_end".to_string());

        let report_path = app
            .finalize_teaching_session()
            .expect("finalize teaching session")
            .expect("teaching mode writes report");
        ...
        .expect("read jsonl");
        assert!(jsonl.contains("\"type\":\"session-ended\""));
        let report = std::fs::read_to_string(report_path).expect("read report");
        assert!(report.contains("session.ended"));
    }

    #[test]
    fn finalize_teaching_session_skips_when_teaching_disabled() {
        let dir = tempfile::tempdir().expect("tempdir");
        let mut options = test_options(false);
        options.workspace = dir.path().to_path_buf();
        let mut app = App::new(options, &Config::default());
        ...
        let result = app
            .finalize_teaching_session()
            .expect("finalize non-teaching session");
