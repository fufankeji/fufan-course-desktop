...
#[test]
fn teaching_sidebar_visibility_requires_student_event_and_wide_area() {
    ...
    state.push(crate::education::events::EducationEvent::new(
        "evt_teacher",
        "sess_1",
        crate::education::events::EducationEventType::ApprovalRequested,
        "2026-06-17T15:00:00+08:00",
        crate::education::events::EducationActor::System,
        crate::education::events::EducationVisibility::Teacher,
        crate::education::events::EducationSeverity::Warning,
        "teacher-only event",
        serde_json::json!({}),
    ));
    assert!(!teaching_sidebar_visible(true, &state, 160));
...
#[test]
fn teaching_sidebar_split_uses_right_thirty_percent_without_existing_sidebar() {
    let mut state = crate::education::state::EducationState::default();
    state.push(crate::education::events::EducationEvent::new(
        "evt_student",
        "sess_1",
    ...
    let area = Rect::new(0, 0, 150, 24);
    let (chat_area, teaching_area) =
        split_teaching_sidebar_for_chat_area(area, true, &state, false);

    assert_eq!(chat_area, Rect::new(0, 0, 105, 24));
    assert_eq!(teaching_area, Some(Rect::new(105, 0, 45, 24)));

    let (chat_area, teaching_area) =
        split_teaching_sidebar_for_chat_area(area, false, &state, false);
    assert_eq!(chat_area, area);
    assert_eq!(teaching_area, None);

    let (chat_area, teaching_area) = split_teaching_sidebar_for_chat_area(area, true, &state, true);
    assert_eq!(chat_area, area);
    assert_eq!(teaching_area, None);
}
